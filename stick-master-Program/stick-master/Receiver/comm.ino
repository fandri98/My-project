//** serial comm
// call `serial_receive()` in the loop
...
void encode_data(){
  ...
  if(head == 0x10){
   BUTTON1     = serial_buffer[1];
   BUTTON2     = serial_buffer[2];
   angle       = serial_buffer[3] | (serial_buffer[4]<<8);
   magnitude   = serial_buffer[5];
   omega       = serial_buffer[6];
  }
}
...
void vib(uint8_t mag)
{
  uint8_t data[2] = {0x11, mag};
  serial_send_packets( data, 2);
}
